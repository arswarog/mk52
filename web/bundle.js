...
var MKDisplay = /** @class */ (function () {
    function MKDisplay(register) {
        ...
        if (register instanceof register_1.Register) {
            ...
            var magn = register.magn + register.magnToNormal;
            if (register.mant == 0) {
                this.mode = DisplayMode.Simple;
                this.simple();
            }
    ...
    MKDisplay.prototype.normal = function () {
        ...
        var sign = this.register.magn + delta;
        if (sign < 0)
            this.signMinus = true;
    ...
    MKDisplay.prototype.numToDigits = function (value, dotted) {
        if (dotted === void 0) { dotted = true; }
        var mant = [];
        var str = '' + value;
        var haveDot = false;
        var nums = 0;
        for (var i = 0; nums < 9 && i < str.length; i++) {
            if (str[i] === '.') {
                haveDot = true;
                mant.push(register_1.Digit.dot);
            }
            else {
                mant.push(+str[i]);
                nums++;
        ...
        if (dotted) {
            if (!haveDot)
                mant.push(register_1.Digit.dot);
            while (mant.length && mant[mant.length - 1] === register_1.Digit.h0)
                mant.pop();
        }
        return mant;
    };
...
var Register = /** @class */ (function () {
    ...
    Object.defineProperty(Register.prototype, "magnToNormal", {
        get: function () {
            var delta = 0;
            var mant = this.mant;
     ...
     * @return {Register}
     */
    Register.prototype.div = function (y) {
        var mant = this.mantissa / y.mantissa;
        var magn = this.magnitude - y.magnitude;
        return new Register(mant, magn);
    };
