...
describe('Register', () => {
    describe('intNormalize', () => {
        ...
        it('-0.000000056723', () => {
            const res = intNormalize(-0.000000056723);
            expect(res).toEqual({mantissa: 56723, exp: -12, negative: true});
        });
        it('5.2 * 10**-63', () => {
            const res = intNormalize(5.2 * 10 ** -63);
    ...
    describe('fromExponential', () => {
        it('0e+0', () => {
            const register = Register.fromExponential('0e+0');
            expect(register.negative).toBeFalsy();
            expect(register.mantissa).toBe(0);
            expect(register.exp).toBe(0);
        });
        it('0.00e+0', () => {
            const register = Register.fromExponential('0.00e+0');
            expect(register.negative).toBeFalsy();
            expect(register.mantissa).toBe(0);
            expect(register.exp).toBe(0);
        });
        it('3.14e+0', () => {
            const register = Register.fromExponential('3.14e+0');
            expect(register.negative).toBeFalsy();
            expect(register.mantissa).toBe(314);
            expect(register.exp).toBe(-2);
        });
        it('8.35363e+27', () => {
            const register = Register.fromExponential('8.35363e+27');
            expect(register.negative).toBeFalsy();
            expect(register.mantissa).toBe(835363);
            expect(register.exp).toBe(22);
        });
        it('-1.0378234e-18', () => {
            const register = Register.fromExponential('-1.0378234e-18');
            expect(register.negative).toBeTruthy();
            expect(register.mantissa).toBe(10378234);
            expect(register.exp).toBe(-25);
        });
    });

    describe('from BigNumber', () => {
        it('0e+0', () => {
            const register = Register.fromBigNumber(new BigNumber('0e+0'));
            expect(register.negative).toBeFalsy();
            expect(register.mantissa).toBe(0);
            expect(register.exp).toBe(0);
        });
        it('0.0e+0', () => {
            const register = Register.fromBigNumber(new BigNumber('0.0e+0'));
            expect(register.negative).toBeFalsy();
            expect(register.mantissa).toBe(0);
            expect(register.exp).toBe(0);
        });
        it('3.14e+0', () => {
            const register = Register.fromBigNumber(new BigNumber('3.14e+0'));
            expect(register.negative).toBeFalsy();
            expect(register.mantissa).toBe(314);
            expect(register.exp).toBe(-2);
        });
        it('8.35363e+27', () => {
            const register = Register.fromBigNumber(new BigNumber('8.35363e+27'));
            expect(register.negative).toBeFalsy();
            expect(register.mantissa).toBe(835363);
            expect(register.exp).toBe(22);
        });
        it('-1.0378234e-18', () => {
            const register = Register.fromBigNumber(new BigNumber('-1.0378234e-18'));
            expect(register.negative).toBeTruthy();
            expect(register.mantissa).toBe(10378234);
            expect(register.exp).toBe(-25);
        });
